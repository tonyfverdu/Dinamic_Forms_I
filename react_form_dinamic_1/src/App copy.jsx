...
function App() {
  ...
  */
  ...
  const handleSave = async (parJSON) => {
    console.log("parJSON en handleSave: ", parJSON)
    const newData = parJSON; /* dato nuevo */
    await fetch('http://127.0.0.1:3001/api/datasave/', {
      method: 'POST',
      headers: {
  ...
  function handleButtonUpdateUser() {
  ...
  return (
    <>
      <div className="buttons_ini">
        ...
        />
        <ButtonKlein
          handleButton={handleButtonGetUser}
          text="Updated User"
          parW="12rem"
          parH="2.6rem"
          parFS="1rem"
      ...
      <form className="form" onSubmit={(ev) => handleFormSubmit(ev)}>
        ...
        <div className="reg_form">
          <SelectElement
            ...
            value={provincia || data_submit["provincia"]}
          ...
          <SelectElement
            ...
            value={data_submit["poblacion"]}
            defaultValue={data_submit["poblacion"]}
          ...
          <SelectElement
            ...
            response={[""]}
            optionsValues={casos}
            value={data_submit["tipo_caso"]}
            defaultValue={data_submit["tipo_caso"]}
            onChange={setCaso}
          // setValue={setCaso}
          />
...
function SelectElement({ select_id, selectTextLabel, required, disabled, response, optionsValues, value, onChange }) {
...
function DataSubmit({ data_submit }) {
  return (
    <div>
      ...
      <p>{data_submit.mail}<span>{"  -  "}</span><span>{data_submit.tel}</span></p>
      <p>{data_submit.provincia}<span>{"  -  "}</span><span>{data_submit.poblacion}</span></p>
      <p>{data_submit.tipo_caso}</p>
      <p>{data_submit.country}</p>
      <p>{data_submit.ccaa}</p>
    </div>
  )
}
